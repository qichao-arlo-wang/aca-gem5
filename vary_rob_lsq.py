...
for i, rob_size in enumerate(sizes):
    for lsq_size in sizes:
        ...
        subprocess.run("python /homes/lp721/aca-gem5/simulate.py --rob-size "+str(rob_size)+" --num-int-phys-regs "+str(phys_regs)+" --num-float-phys-regs "+str(phys_regs)+" --num-vec-phys-regs "+str(phys_regs)+" --lsq-size "+str(lsq_size)+" --name "+name, shell=True)
