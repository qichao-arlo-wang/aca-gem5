...
for i, rob_size in enumerate(sizes):
    for lsq_size in sizes:
        ...
        subprocess.run("python simulate.py --rob-size "+str(rob_size)+" --num-int-phys-regs "+str(int_phys_regs)+" --num-float-phys-regs "+str(rob_size)+" --num-vec-phys-regs "+str(rob_size)+" --lsq-size "+str(lsq_size)+" --name "+name, shell=True) 
