...
def replace_pipe(filename):
  """ Function to replace the pipe symbols in the stat file generated by Gem5.
  This is specific for the capability cache use case.
  """
...
def readStatsFile(statsFile):
    global stats
    stats = {}
    replace_pipe(statsFile) # remove the pipe symbols introduced by the histogram feature
    F = open(statsFile)
    ignores = re.compile(r'^---|^$')
    statLine = re.compile(r'([a-zA-Z0-9_\.:-]+)\s+([-+]?[0-9]+\.[0-9]+|[-+]?[0-9]+|nan|inf)')
    # statLine_hist = re.compile(r"([a-zA-Z0-9_\.:-]+)\s+(\|)\s+([-+]?[0-9]+\.[0-9]+|[-+]?[0-9]+|nan|inf)")
    count = 0
    # debug_count = 0
    for line in F:
        ...
        if not ignores.match(line):
            ...
            # print(f"{debug_count} - {statKind} - {statValue}")
            # debug_count += 1
    F.close()
