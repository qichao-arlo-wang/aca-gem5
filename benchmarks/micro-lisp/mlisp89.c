...
Object *eval_string(const char *str, Object *env) {
  Object *port = newport( fmemopen(str, strlen(str)+1, "r") );
  lookahead(port);
...
int main(int argc, char *argv[]) {
  ...
  DIR *pDir;
  if (argc < 2) { printf("Please specify example dir\n"); return 1; }
  pDir = opendir(argv[1]);
  if (!pDir) { printf("Failed to open example dir\n"); return 1; }
  if (argv[1][strlen(argv[1])] != '/') strcat(argv[1], "/");
  FILE *in = stdin;
  char lisp_input[1000];
  strcpy(lisp_input, argv[1]);
  while ((pDirent = readdir(pDir))) {
    size_t file_len = strlen(pDirent->d_name);
